...
- **[gen]** Mcp at 0. Remaining tail = ONE def-side class: zod-Infer aliases emitted as `= obj` abbreviations while refs treat them as types (one FS0887 inherit, one FS0033 over-application) — designed fix: emit obj-degraded aliases as EMPTY INTERFACES (the existing abbreviation-verdict idiom), measured fresh, NOT a tail bolt-on (a heritage-side IR filter was tried 2026-07-05 and REVERTED: dropped 40 healthy inherits without fixing the target). *Closure: `dotnet build` exit 0 on the Mcp unit.*
- **[gen]** CodeMode and Agents at 0 (first real counts unblock at Mcp 0). *Closure: per-unit build exit 0; Phase-1 exit = the STATUS matrix all-zero.*
...
- **[gen]** DEFERRED-TOGETHER pair (measured 2026-07-05, evidence in the baseline): scrub-host correction for holder params (surfaces real refs — fetch renders true `Request`/`URL` types instead of `obj`) + the cross-owner def/ref closure (cache-hit literals' defs materialize only at their first owner — the `ExportedHandler.Fetch` / 120B-`U2<obj,obj>` family). *Closure: params scrub at the export-unit host with zero new dangles; ledgered obj-degradations on holder params burn down.*
...
- **[gen]** HelloWorker: fetch handler + KV/R2/D1 round-trip, Fable-compiled, runs under miniflare. *Closure: miniflare exit 0 + correct response bytes.* **Unblocked NOW — needs only the Workers unit (assembles since 2026-07-05); the earliest signal on the runtime `Emit`/`Import` fidelity unknown. Do not gate on Mcp.**
- **[gen]** Minimal Agent slice consumer: `Agent` subclass + `routeAgentRequest` + `getAgentByName` + state + `this.sql`, compiled through the bindings. *Closure: Fable compile exit 0; runs under miniflare.* (Gated on the Mcp→CodeMode→Agents chain.)
