...
- [ ] **[gen]** Type-granular shared placement (interval algorithm: host ∈ [max(refs' units), min(referencers' units)]) replacing first-unit pooling; characterize the 5 stray back-edges; SharedLiterals homes re-rooted per hosting unit at prerender. *Closure: pool = 0; measured forward-reference debt only in the zod-policy class.*
