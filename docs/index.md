...
Output targets **Fable 5.x only**, and depends on `Fable.Core`, `Xantham.Fable.Core`, and
`Xantham.Fable.Core.TS`. DOM references use the in-house bindings generated from the pinned
compiler's own `lib.*.d.ts`. Progress is measured on a ladder of real npm
...
| `src/Xantham.Fable.Core.TS` | The generated ECMAScript and DOM bindings. |
...
| **Generator** (`Xantham.Generator`) | 🟡 Alpha | Phases A–C landed; phase D (erased idioms) is most of the way through. Ships as the `Xantham.Cli` dotnet tool at `0.1.0-alpha.1` — see [Generating F# bindings](generator-usage.md). |
