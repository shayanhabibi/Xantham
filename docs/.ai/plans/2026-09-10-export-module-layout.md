...
2. Constraint-only or alias-only collapse: two different declarations whose parameters and return normalize to the same F# signature consolidate to one member with an Ergonomic consolidation finding naming both source declarations. The erased constraint is information the member's own mapping findings already record.
3. Legal overloads: retain unchanged when their compiled parameter signatures are distinct and focused consumer call probes can select them. Do not rename every overload of a function merely because one pair conflicts. A literal-typed parameter that separates an overload set keeps its literal type, exactly as `Overloads.dedupeOverloads` does for interface members; the export path shares that synthesis.
4. Return-only collision: candidates whose parameters normalize to the same F# signature but whose returns differ consolidate to one member returning the erased union of every distinct return, in harvest order, flattened and deduplicated after alias normalization. The renderer's on-demand `U<n>` footer covers arities above nine. The finding is `ExportReturnTypesUnioned` at Widened. Collisions created by arity repair are return-only collisions once repair has run and take the same path.
5. Ambiguous or member-kind collisions: retain every distinct candidate under different F# names. This covers optional/rest overloads whose overlapping call forms cannot be selected reliably and method/property name conflicts. Distinct original signatures are not proven equivalent merely because their mapped return types also agree.
6. Widening: beyond the return union in case 4, introduce **no additional widening in this pass**. Existing mapping widenings retain their original findings; a rename or union finding is not a claim that the widened signature became Exact. Dropping candidates or introducing an `obj` fallback is outside this implementation contract.

Corrected 2026-09-12: the original contract renamed return-only pairs `<name>_Overload2`. Two F# names for one JavaScript function with identical parameters give the call site no way to choose between them, so the union return replaced renaming for that case.
...
Append `ExportOccurrenceConsolidated` (Exact or Ergonomic by case), `ExportMemberRenamed` (Ergonomic) and `ExportReturnTypesUnioned` (Widened) cases to the existing `DedupeOverloads` finding union, with message arms and snapshot updates. Preserve all old case positions and old codes; old export-drop codes remain defined even if unused by the new export path. Include owner, original export name, final member name and reason in findings. Distinguish return-only, constraint/alias collapse, ambiguous call form, and member-kind collision in messages. Manifest entries for renamed or unioned overloads must be individually attributable.

**Finding attribution:** every finding raised for an export member uses the member's container-qualified name, `Exports.check()` or `Strict.Exports.check(value)`, identical to its manifest row. The owner-tagged pseudo-symbols `entry`, `global` and `ambient:<specifier>` are retired, so a member's manifest tier follows its own findings.
