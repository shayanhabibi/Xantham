...
`SI002` when it has no F# name at this position (`extends Error`), `SI005` when it names
something outside this run (`JS.Promise`, a shipped binding this run cannot prove is an
interface, where `inherit` would be FS0887), `SI006` when the edge would close a cycle in the
...
application and is written as the constraint instead (`TR037`).
