...
| **Phase M — TS lib.es5 utility-type aliases, iterator types, and lib.dom event-map / iterator / messaging globals in the substitution table** | The render layer's `LibEsDefaults.substitutions` table is the documented seam between TS lib.es type names and their F# equivalents. TS lib.es5 utility-type aliases (`Omit`, `Pick`, `Partial`, `Required`, `Readonly`, `Exclude`, `Extract`, `NonNullable`, `ReturnType`, `Parameters`, `ConstructorParameters`, `InstanceType`, `Awaited`, `ThisParameterType`, `OmitThisParameter`, `ThisType`, `NoInfer`; plus the four string-transform intrinsics `Uppercase`/`Lowercase`/`Capitalize`/`Uncapitalize`) had no entries — references emitted as bare path-rooted names (`Omit<X, K>`) which F# couldn't resolve. At HERITAGE positions these were the dominant pattern: `interface X extends Partial<Omit<Y, K>>` produced `inherit option<Omit<obj, obj>>` after cycle-break (Partial → option<…>; Omit args broken to obj). The existing heritage filter (`isObjOrExnIntrinsic` in `TypeRender.Render.fs`) only catches bare `obj`/`exn` atoms, not Prefix molecules — so cycle-broken-but-still-named aliases survived as orphan references, producing paired FS0039 (`'Omit' is not defined`) + FS0887 (`'obj' is not an interface type`) at every site. Phase H closes this at NON-heritage TypeNode positions via `getTypeOfSymbol`, but heritage refs route through `Class.exprWithTypeArgsToRefSlot` (a different path that Phase H does not touch). Fix: substitute the utility-type aliases to `obj` arity 0 (string for the four string-transform intrinsics). At heritage, refs become bare `obj` and the existing filter drops them; at member-position they collapse to `obj` (lossy on structural intent, compilation-correct). Same pattern extended to TS 5.x iterator additions (`SetIterator`, `MapIterator`, `ArrayIterator`, `RegExpStringIterator`, `RegExpMatchArray`, `RegExpExecArray`, `TemplateStringsArray`, `ReadonlySetLike`, `WeakKey`, `PromisifyFn`); lib.dom event-map types (`WebSocketEventMap`, `WindowEventMap`, `HTMLElementEventMap`, etc. — the `K extends keyof TheEventMap`–lookup-dictionary pattern used with `addEventListener`); lib.es2015.iterable result types (`IteratorYieldResult`/`IteratorReturnResult`/`IteratorResult`); lib.dom iteration-protocol wrappers (`URLSearchParamsIterator`/`HeadersIterator`/`FormDataIterator`); and lib.dom messaging / window globals (`Window`/`MessagePort`/`MessageChannel`/`BroadcastChannel`/`Storage`/`Location`). All emit as `obj` arity 0 — F# stdlib has no analogue, and consumers refine via `Fable.Browser.*` bindings at use sites if structural detail is needed. The principle: when an F# binding diverges from TS — by name, arity, or absence — the substitution table is the seam, not a per-site special case. Phase H handles the "TS computes a structural body" case; Phase M handles the "TS-name has no F# binding" case at every position including heritage | (Phase M) |
| **Phase N — Collapse uncanonicalised Intersection / TypeLiteral synthetics to `obj`** | `SyntheticPathAssignment.run` walks exports and assigns canonical `TypePath`s for Union / Intersection / TypeLiteral / TemplateLiteral synthetic literals. Reference sites consult `ctx.SyntheticPaths` and emit the canonical path. The fallthrough — for synthetics NOT in `ctx.SyntheticPaths` — was constructing a *transient* path from the local `scope.PathContext` and recording the body's emission at that transient location. Two failure modes resulted: (a) when the *same* synthetic is reached via different `ResolvedType` identities (decoder doesn't intern by structural equality, `[<ReferenceEquality>]` makes structurally-equivalent intersections distinct), each call site emits its own transient body at its own scope, and a third call site's reference (constructed from yet another scope) targets neither emission; (b) IndexSignature parameter and return positions construct scopes like `<Owner>.Item.<ParamPascal>` and `<Owner>.Item.Item` — the body emits at the cached location but `RootModule.collectModules` doesn't reliably surface it at every reference site's constructed path. Both produced dangling-reference cohorts (`'Options' is not defined`, `'Item' is not defined in '...Item'`, `'WorkflowName.Item.Key' is not defined`, etc.) at thousands of sites across the 8 large SDKs. Fix: in `RenderScope.Prelude.fs`'s Intersection and TypeLiteral non-inline fallthrough branches, replace the transient-path construction with `liftNullable Intrinsic.obj`. Synthetics that DID get canonical paths from `SyntheticPathAssignment` continue through their existing branches unchanged — the change is bounded to "intersections/literals that escaped canonicalisation." Lossy on structural detail (those synthetics' members are now invisible to consumers), but the loss is bounded and compilation-correct. **Note:** Phase N is a renderer-side stabiliser; the actual root cause is the decoder's reference-identity drift on structurally-equivalent types (and the encoder's UnknownDeclared classifier — see Phase O) — see [[trace-symptom-back-to-encoder]]. The principle: the canonical-path system is the source of truth; emissions should never construct alternative paths that compete with it | (Phase N) |
| **Phase O — `UnknownDeclared` source accepted by substitution gate** | `LibEsDefaults.tryLookupSubstitution` and `resolvedTypePreludeInterceptor` gated the substitution table on `Source.LibEs _` only. But the encoder's classifier in `TypeDeclaration.fs:467` assigns `Source.UnknownDeclared` as a fallback when a declaration can't be attributed to a known package or recognised as a TS standard-library declaration. Many lib.dom types end up here because they come from consumer packages' fetch.d.ts / similar shims (e.g. `MessageEventInit` at `@cloudflare/workers-types/dist/fetch.d.ts` has `Source = UnknownDeclared "fetch.d.ts"` in the encoded JSON, NOT `Source = LibEs`). The substitution gate then skipped these even though their names (`MessageEventInit`, `ServiceWorker`, `WebSocketEventMap`, etc.) were in the table — the body emitted as a full `[<AbstractClass>] type X private () = inherit obj() ... abstract member` with references to neighbouring un-substituted lib types that then dangled as FS0039. Fix: extend `isLibEsSource` to also accept `Source.UnknownDeclared _` and add corresponding `when isSubstitutionSource source` guards to the interceptor patterns. Substitution-table entries now fire for both LibEs-attributed and UnknownDeclared-attributed types; `Source.Package _` (real consumer-package augmentation) continues to keep its full body. Traced from F# symptom (`'ServiceWorker' is not defined`) → encoded JSON Source field (`UnknownDeclared:"fetch.d.ts"`) → gate logic. This is the second-layer root-cause repair; the encoder classifier itself could be improved to attribute these lib-dom shims as `Source.LibEs`, but the gate-widening is a less-invasive fix and the symptom + cure both live at the substitution seam | (Phase O) |
