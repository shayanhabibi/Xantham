...
- `src/Xantham.Fable.Node` — generated `@types/node` bindings, published to NuGet and consumed
  alongside `Xantham.Fable.Core.TS`; regenerated by `generate --only node-lib`.
...
- `tests/Xantham.Generator.CompileGate` — not a test project: compiles the goldens on every build.
- `tests/Xantham.Generator.CompilerLibCompileGate` — compiles the compiler-lib ownership lab
  under its ES5 profile, which owns `TypeScript.Lib.Es` independently of the main compile gate.
- `tests/Xantham.Generator.RunGate` — Fable-runs behavioural checks against a subset of the
  goldens. Opt-in: a plain build compiles it as .NET only, and the Fable run happens under
  `build.fsx -- test --run-gate`, locally and in the CI test workflow
...
- `dotnet build Xantham.slnx` — build.
- `dotnet fsi build.fsx -- test [--quick] [--update] [--filter <name>] [--run-gate]` — run the
  Expecto suites. Use this, not `dotnet test`, which stalls on the suites' nested builds
  (`docs/.ai/footguns.md`). `--quick` skips setup, `--update` rewrites the goldens, `--filter`
...
- Findings are graded `Exact | Ergonomic | Widened | Escape` per symbol into `manifest.json`, so a
  ...
  `SI001`) comes from `FindingCodes.table` in `Findings.fs`, not from case position: a new case
