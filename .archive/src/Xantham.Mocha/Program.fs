...
module Nib =
    ...
    let inline contains<^a, ^b when ^a:equality and ^b :> ^a seq> (expected: ^a): Assertion<^b> =
        assertion (fun b -> b :> ^a seq |> Seq.contains expected) (sprintf "Expected collection to contain element %A, but got none.\n\n%A" expected)
    let inline notContain<^a, ^b when ^a:equality and ^b :> ^a seq> (expected: ^a): Assertion<^b> =
        assertion (fun b -> b :> ^a seq |> Seq.contains expected |> not) (sprintf "Expected collection to not contain element %A, but got it.\n\n%A" expected)
