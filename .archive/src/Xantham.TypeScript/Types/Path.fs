...
let inline dir path = Node.Api.path.dirname path |> Measures.annotate<Measures.directory>
...
let relativeNodeModules (path: string) =
    ...
    |> Measures.annotate<Measures.nodeModulesRelative>
...
let inferredPackageDirectory (path: string) =
    if not (hasNodeModules path) then Node.Api.path.dirname path |> Measures.annotate<Measures.packageDirectory> else
    let pathParts =
    ...
    |> Measures.annotate<Measures.packageDirectory>
...
let extensionless path =
    ...
    |> Measures.annotate<Measures.extensionless>