...
module Ts =
    [<Fable.Core.Import("ScriptSnapshot","typescript")>]
    ...
    type [<AllowNullLiteral>] IExports =
        ...
        abstract isPartOfTypeOnlyImportOrExportDeclaration: node: Node -> bool
        ...
        abstract isDeclarationStatement: node: Node -> bool
        ...
        abstract isJsxCallLike: node: Node -> bool
        ...
        abstract isInternalDeclaration: node: Node * ?sourceFile: SourceFile -> bool
        abstract unchangedTextChangeRange: TextChangeRange
        abstract isPartOfTypeNode: node: Node -> bool
        /// <summary>
        ...
        abstract isModuleExportName: node: Node -> bool
        ...
        abstract isJSDocImportTag: node: Node -> bool
        ...
        abstract parseBuildCommand: commandLine: ResizeArray<string> -> ParsedBuildCommand
        ...
        abstract visitEachChild: node: 'T * visitor: Visitor * context: TransformationContext option -> 'T when 'T :> Node
        /// <summary>Visits each child of a Node using the supplied visitor, possibly returning a new Node of the same kind in its place.</summary>
        /// <param name="node">The Node whose children will be visited.</param>
        /// <param name="visitor">The callback used to visit each child.</param>
        /// <param name="context">A lexical environment context for the visitor.</param>
        abstract visitEachChild: node: 'T option * visitor: Visitor * context: TransformationContext option * ?nodesVisitor: obj * ?tokenVisitor: Visitor -> 'T option when 'T :> Node
        ...
        /// <summary>
        /// Use <c>program.getModeForResolutionAtIndex</c>, which retrieves the correct <c>compilerOptions</c>, instead of this function whenever possible.
        /// Calculates the final resolution mode for an import at some index within a file's <c>imports</c> list. This is the resolution mode
        /// explicitly provided via import attributes, if present, or the syntax the usage would have if emitted to JavaScript. In
        /// <c>--module node16</c> or <c>nodenext</c>, this may depend on the file's <c>impliedNodeFormat</c>. In <c>--module preserve</c>, it depends only on the
        /// input syntax of the reference. In other <c>module</c> modes, when overriding import attributes are not provided, this function returns
        /// <c>undefined</c>, as the result would have no impact on module resolution, emit, or type checking.
        /// </summary>
        ...
        /// </param>
        abstract getModeForResolutionAtIndex: file: SourceFile * index: float * compilerOptions: CompilerOptions -> ResolutionMode
        /// <summary>
        /// Use <c>program.getModeForUsageLocation</c>, which retrieves the correct <c>compilerOptions</c>, instead of this function whenever possible.
        ...
        abstract getModeForUsageLocation: file: SourceFile * usage: StringLiteralLike * compilerOptions: CompilerOptions -> ResolutionMode
        ...
        /// </summary>
        /// <param name="fileName">The file name to check the format of (it need not exist on disk)</param>
        ...
        abstract getImpliedNodeFormatForFile: fileName: string * packageJsonInfoCache: PackageJsonInfoCache option * host: ModuleResolutionHost * options: CompilerOptions -> ResolutionMode
        /// <summary>
        ...
        abstract isBuildCommand: commandLineArgs: ResizeArray<string> -> bool
        ...
        abstract transpileDeclaration: input: string * transpileOptions: TranspileOptions -> TranspileOutput
    ...
    /// </code>
    /// </remarks>
    type TriviaSyntaxKind = obj
    [<RequireQualifiedAccess>]
    module TriviaSyntaxKind =
        let [<Literal>] SingleLineCommentTrivia: TriviaSyntaxKind = SyntaxKind.SingleLineCommentTrivia
        let [<Literal>] MultiLineCommentTrivia: TriviaSyntaxKind = SyntaxKind.MultiLineCommentTrivia
        let [<Literal>] NewLineTrivia: TriviaSyntaxKind = SyntaxKind.NewLineTrivia
        let [<Literal>] WhitespaceTrivia: TriviaSyntaxKind = SyntaxKind.WhitespaceTrivia
        let [<Literal>] ShebangTrivia: TriviaSyntaxKind = SyntaxKind.ShebangTrivia
        let [<Literal>] ConflictMarkerTrivia: TriviaSyntaxKind = SyntaxKind.ConflictMarkerTrivia
    ...
    type PunctuationSyntaxKind = Enum
    [<RequireQualifiedAccess>]
    module PunctuationSyntaxKind =
        let [<Literal>] OpenBraceToken: PunctuationSyntaxKind = SyntaxKind.OpenBraceToken
        let [<Literal>] CloseBraceToken: PunctuationSyntaxKind = SyntaxKind.CloseBraceToken
        let [<Literal>] OpenParenToken: PunctuationSyntaxKind = SyntaxKind.OpenParenToken
        let [<Literal>] CloseParenToken: PunctuationSyntaxKind = SyntaxKind.CloseParenToken
        let [<Literal>] OpenBracketToken: PunctuationSyntaxKind = SyntaxKind.OpenBracketToken
        let [<Literal>] CloseBracketToken: PunctuationSyntaxKind = SyntaxKind.CloseBracketToken
        let [<Literal>] DotToken: PunctuationSyntaxKind = SyntaxKind.DotToken
        let [<Literal>] DotDotDotToken: PunctuationSyntaxKind = SyntaxKind.DotDotDotToken
        let [<Literal>] SemicolonToken: PunctuationSyntaxKind = SyntaxKind.SemicolonToken
        let [<Literal>] CommaToken: PunctuationSyntaxKind = SyntaxKind.CommaToken
        let [<Literal>] QuestionDotToken: PunctuationSyntaxKind = SyntaxKind.QuestionDotToken
        let [<Literal>] LessThanToken: PunctuationSyntaxKind = SyntaxKind.LessThanToken
        let [<Literal>] LessThanSlashToken: PunctuationSyntaxKind = SyntaxKind.LessThanSlashToken
        let [<Literal>] GreaterThanToken: PunctuationSyntaxKind = SyntaxKind.GreaterThanToken
        let [<Literal>] LessThanEqualsToken: PunctuationSyntaxKind = SyntaxKind.LessThanEqualsToken
        let [<Literal>] GreaterThanEqualsToken: PunctuationSyntaxKind = SyntaxKind.GreaterThanEqualsToken
        let [<Literal>] EqualsEqualsToken: PunctuationSyntaxKind = SyntaxKind.EqualsEqualsToken
        let [<Literal>] ExclamationEqualsToken: PunctuationSyntaxKind = SyntaxKind.ExclamationEqualsToken
        let [<Literal>] EqualsEqualsEqualsToken: PunctuationSyntaxKind = SyntaxKind.EqualsEqualsEqualsToken
        let [<Literal>] ExclamationEqualsEqualsToken: PunctuationSyntaxKind = SyntaxKind.ExclamationEqualsEqualsToken
        let [<Literal>] EqualsGreaterThanToken: PunctuationSyntaxKind = SyntaxKind.EqualsGreaterThanToken
        let [<Literal>] PlusToken: PunctuationSyntaxKind = SyntaxKind.PlusToken
        let [<Literal>] MinusToken: PunctuationSyntaxKind = SyntaxKind.MinusToken
        let [<Literal>] AsteriskToken: PunctuationSyntaxKind = SyntaxKind.AsteriskToken
        let [<Literal>] AsteriskAsteriskToken: PunctuationSyntaxKind = SyntaxKind.AsteriskAsteriskToken
        let [<Literal>] SlashToken: PunctuationSyntaxKind = SyntaxKind.SlashToken
        let [<Literal>] PercentToken: PunctuationSyntaxKind = SyntaxKind.PercentToken
        let [<Literal>] PlusPlusToken: PunctuationSyntaxKind = SyntaxKind.PlusPlusToken
        let [<Literal>] MinusMinusToken: PunctuationSyntaxKind = SyntaxKind.MinusMinusToken
        let [<Literal>] LessThanLessThanToken: PunctuationSyntaxKind = SyntaxKind.LessThanLessThanToken
        let [<Literal>] GreaterThanGreaterThanToken: PunctuationSyntaxKind = SyntaxKind.GreaterThanGreaterThanToken
        let [<Literal>] GreaterThanGreaterThanGreaterThanToken: PunctuationSyntaxKind = SyntaxKind.GreaterThanGreaterThanGreaterThanToken
        let [<Literal>] AmpersandToken: PunctuationSyntaxKind = SyntaxKind.AmpersandToken
        let [<Literal>] BarToken: PunctuationSyntaxKind = SyntaxKind.BarToken
        let [<Literal>] CaretToken: PunctuationSyntaxKind = SyntaxKind.CaretToken
        let [<Literal>] ExclamationToken: PunctuationSyntaxKind = SyntaxKind.ExclamationToken
        let [<Literal>] TildeToken: PunctuationSyntaxKind = SyntaxKind.TildeToken
        let [<Literal>] AmpersandAmpersandToken: PunctuationSyntaxKind = SyntaxKind.AmpersandAmpersandToken
        let [<Literal>] AmpersandAmpersandEqualsToken: PunctuationSyntaxKind = SyntaxKind.AmpersandAmpersandEqualsToken
        let [<Literal>] BarBarToken: PunctuationSyntaxKind = SyntaxKind.BarBarToken
        let [<Literal>] BarBarEqualsToken: PunctuationSyntaxKind = SyntaxKind.BarBarEqualsToken
        let [<Literal>] QuestionQuestionToken: PunctuationSyntaxKind = SyntaxKind.QuestionQuestionToken
        let [<Literal>] QuestionQuestionEqualsToken: PunctuationSyntaxKind = SyntaxKind.QuestionQuestionEqualsToken
        let [<Literal>] QuestionToken: PunctuationSyntaxKind = SyntaxKind.QuestionToken
        let [<Literal>] ColonToken: PunctuationSyntaxKind = SyntaxKind.ColonToken
        let [<Literal>] AtToken: PunctuationSyntaxKind = SyntaxKind.AtToken
        let [<Literal>] BacktickToken: PunctuationSyntaxKind = SyntaxKind.BacktickToken
        let [<Literal>] HashToken: PunctuationSyntaxKind = SyntaxKind.HashToken
        let [<Literal>] EqualsToken: PunctuationSyntaxKind = SyntaxKind.EqualsToken
        let [<Literal>] PlusEqualsToken: PunctuationSyntaxKind = SyntaxKind.PlusEqualsToken
        let [<Literal>] MinusEqualsToken: PunctuationSyntaxKind = SyntaxKind.MinusEqualsToken
        let [<Literal>] AsteriskEqualsToken: PunctuationSyntaxKind = SyntaxKind.AsteriskEqualsToken
        let [<Literal>] AsteriskAsteriskEqualsToken: PunctuationSyntaxKind = SyntaxKind.AsteriskAsteriskEqualsToken
        let [<Literal>] SlashEqualsToken: PunctuationSyntaxKind = SyntaxKind.SlashEqualsToken
        let [<Literal>] PercentEqualsToken: PunctuationSyntaxKind = SyntaxKind.PercentEqualsToken
        let [<Literal>] LessThanLessThanEqualsToken: PunctuationSyntaxKind = SyntaxKind.LessThanLessThanEqualsToken
        let [<Literal>] GreaterThanGreaterThanEqualsToken: PunctuationSyntaxKind = SyntaxKind.GreaterThanGreaterThanEqualsToken
        let [<Literal>] GreaterThanGreaterThanGreaterThanEqualsToken: PunctuationSyntaxKind = SyntaxKind.GreaterThanGreaterThanGreaterThanEqualsToken
        let [<Literal>] AmpersandEqualsToken: PunctuationSyntaxKind = SyntaxKind.AmpersandEqualsToken
        let [<Literal>] BarEqualsToken: PunctuationSyntaxKind = SyntaxKind.BarEqualsToken
        let [<Literal>] CaretEqualsToken: PunctuationSyntaxKind = SyntaxKind.CaretEqualsToken
    /// <remarks>
    /// Original in TypeScript:
    /// <code lang="typescript">
...
This method is a companion for 'resolveModuleNames' and is used to resolve 'types' references to actual type declaration files")>]
    ...
    type [<AllowNullLiteral>] UserPreferences =
        ...
        /// <summary>
        /// Indicates whether <see cref="CompletionEntry.labelDetailscompletion">entry label details</see> are supported.
    ...
    type [<AllowNullLiteral>] ParsedBuildCommand =
        abstract buildOptions: BuildOptions with get, set
        abstract watchOptions: WatchOptions option with get, set
        abstract projects: ResizeArray<string> with get, set
        abstract errors: ResizeArray<Diagnostic> with get, set
    ...
    type [<AllowNullLiteral>] WatchHost =
        ...
        abstract clearTimeout: timeoutId: obj option -> unit
        abstract preferNonRecursiveWatch: bool option with get, set
...
If provided, used to resolve type reference directives, otherwise typescript's default resolution")>]
    ...
    type [<AllowNullLiteral>] BuildOptions =
        ...
        abstract stopBuildOnErrors: bool option with get, set
        abstract incremental: bool option with get, set
        abstract assumeChangesOnlyAffectDirectDependencies: bool option with get, set
        abstract declaration: bool option with get, set
        abstract declarationMap: bool option with get, set
    ...
    type [<AllowNullLiteral>] LanguageService =
        ...
        /// <param name="maximumLength">Maximum length of a quickinfo text before it is truncated.</param>
        abstract getQuickInfoAtPosition: fileName: string * position: float * ?maximumLength: float -> QuickInfo option
        ...
        abstract getNavigateToItems: searchValue: string * ?maxResultCount: float * ?fileName: string * ?excludeDtsFiles: bool * ?excludeLibFiles: bool -> ResizeArray<NavigateToItem>
        ...
        abstract dispose: unit -> unit
        abstract preparePasteEditsForFile: fileName: string * copiedTextRanges: ResizeArray<TextRange> -> bool
        abstract getPasteEdits: args: PasteEditsArgs * formatOptions: FormatCodeSettings -> PasteEdits
    ...
    type [<AllowNullLiteral>] PasteEdits =
    ...
    type [<AllowNullLiteral>] PasteEditsArgs =
        abstract targetFile: string with get, set
        abstract pastedText: ResizeArray<string> with get, set
        abstract pasteLocations: ResizeArray<TextRange> with get, set
        abstract copiedFrom: {| file: string; range: ResizeArray<TextRange> |} option with get, set
    ...
    type [<AllowNullLiteral>] RefactorActionInfo =
        ...
        abstract range: {| start: {| line: float; offset: float |}; ``end``: {| line: float; offset: float |} |} option with get, set
    ...
    type [<AllowNullLiteral>] QuickInfo =
        ...
        abstract canIncreaseVerbosityLevel: bool option with get, set
    ...
    type [<AllowNullLiteral>] CompletionInfo =
        ...
        abstract defaultCommitCharacters: ResizeArray<string> option with get, set
    ...
    type [<AllowNullLiteral>] CompletionEntryLabelDetails =
        ...
        /// <see cref="CompletionEntry.namename" />, without any spacing. Should be
