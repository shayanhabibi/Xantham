...
module InternalTracer =
    type ProgramTracerMap =
        ...
        abstract mapsDirty: bool with get,set
    type UniqueIdentifierMapValue = {| Symbol: Tracer<Symbol.Kind> option; Type: Tracer<Type.Kind> option; Node: Tracer<Type.Kind> option |}
    type UniqueIdentifierMapKey = UniqueIdentifiers.CompositeKey
    ...
    let private symbolNodeMapKey =
        SymbolTypeKey.create<Dictionary<UniqueIdentifiers.SymbolKey, HashSet<UniqueIdentifiers.NodeKey>>> "SymbolNodeMap"
    let private nodeTypeMapKey =
        SymbolTypeKey.create<Dictionary<UniqueIdentifiers.NodeKey, UniqueIdentifiers.TypeKey>> "NodeTypeMap"
    let private typeSymbolMapKey =
        SymbolTypeKey.create<Dictionary<UniqueIdentifiers.TypeKey, UniqueIdentifiers.SymbolKey>> "TypeSymbolMap"
    let private symbolTypeMapKey =
        SymbolTypeKey.create<Dictionary<UniqueIdentifiers.SymbolKey, UniqueIdentifiers.TypeKey>> "SymbolTypeMap"
    let private uniqueIdentifierMapKey =
        SymbolTypeKey.create<Dictionary<UniqueIdentifierMapKey, UniqueIdentifierMapValue>> "UniqueIdentifierMap"
        
    type ProgramTracerMap with
        member inline this.MapsUpdated() = this.mapsDirty <- true
        member inline this.ShouldBatchUpdate = this.mapsDirty
        member inline this.BatchUpdateCompleted() = this.mapsDirty <- false
        ...
        member this.SymbolNodeMap =
            SymbolTypeKey.accessOrInit symbolNodeMapKey (fun () -> Dictionary()) this
        member this.NodeTypeMap =
            SymbolTypeKey.accessOrInit nodeTypeMapKey (fun () -> Dictionary()) this
        member this.TypeSymbolMap =
            SymbolTypeKey.accessOrInit typeSymbolMapKey (fun () -> Dictionary()) this
        member this.SymbolTypeMap =
            SymbolTypeKey.accessOrInit symbolTypeMapKey (fun () -> Dictionary()) this
        member this.UniqueIdentifierMap =
            SymbolTypeKey.accessOrInit uniqueIdentifierMapKey (fun () -> Dictionary()) this
...
module Tracers =
    ...
    type SymbolUniqueESSymbolTracer =
        inherit NarrowedTracer<Kind, Choice<Variable, Property>>
        inherit SymbolTracer
    type SymbolInterfaceTracer =
        inherit NarrowedTracer<Kind, Transient.IInterface>
        inherit SymbolTracer
    ...
    type TypeUniqueESSymbolTracer =
        ...
        inherit TypeTracer
    type TypeInterfaceTracer =
        inherit NarrowedTracer<Type.Kind, Choice<Type.Interface, Type.Class, Type.ClassReference, Type.InterfaceReference>>
        inherit TypeTracer
    ...
    type NodeUniqueESSymbolTracer =
        ...
        inherit NodeTracer
    type NodeInterfaceTracer =
        inherit NarrowedTracer<Node.Kind, Ts.InterfaceDeclaration>
        inherit NodeTracer
...
module rec Core =
    ...
    module Node =
        ...
        module DeclarationKind =
            ...
            let fromExportDeclaration program node = ParentInlinedProgram.wrap program Node.DeclarationKind.ExportDeclaration node
            ...
            let internal kindMap = Dictionary [
                ...
                Ts.SyntaxKind.ExportDeclaration ==> fromExportDeclaration
            ...
            let toNode = function
                ...
                | DeclarationKind.ExportDeclaration exportDeclaration -> exportDeclaration.Value
    ...
    module Symbol =
        ...
        let private kindConstructor: Dictionary<string, (obj -> Symbol.Kind) * (obj -> Transient.Kind)> = Dictionary [
            ...
            nameof GetAccessorDeclaration ==> (Symbol.Kind.GetAccessor </> Transient.Kind.GetAccessor)
            nameof SetAccessorDeclaration ==> (Symbol.Kind.SetAccessor </> Transient.Kind.SetAccessor)
        ...
        let private makeKind (symbol: ISymbol) (declaration: Ts.Declaration) =
            ...
            |> match Node.DeclarationKind.create (ISymbol.program symbol) declaration with
                ...
                | DeclarationKind.GetAccessor _ -> constructKind<GetAccessorDeclaration>
                | DeclarationKind.SetAccessor _ -> constructKind<SetAccessorDeclaration>
                ...
                | DeclarationKind.ExportDeclaration _ -> returnNone
    ...
    module InternalTracer =
        ...
        let inline get (program: Ts.Program) =
            ...
            | ValueNone ->
                ...
                tracer.Imprint
                tracer
        let inline private markDirty program =
            get program
            |> _.MapsUpdated()
        let inline private markClean program =
            get program
            |> _.BatchUpdateCompleted()
        let inline private isDirty program =
            get program
            |> _.ShouldBatchUpdate
        ...
        let inline private getTypeMap program =
            get program
            |> _.TypeMap
        let inline private getSymbolNodeMap program =
            get program
            |> _.SymbolNodeMap
        let inline private getSymbolTypeMap program =
            get program
            |> _.SymbolTypeMap
        let inline private getNodeTypeMap program =
            get program
            |> _.NodeTypeMap
        let inline private getUniqueIdentifierMap program =
            get program
            |> _.UniqueIdentifierMap
        ...
        let getOrAddType (typeTracerFn: Ts.Program -> Ts.Type -> TypeTracer) (program: Ts.Program) (typ: Ts.Type) =
            ...
            | _ ->
                ...
                typeTracer
        let initMaps (program: Ts.Program) =
            let checker = program.getTypeChecker()
            
            let symbolMap = getSymbolMap program
            let symbolNodeMap = getSymbolNodeMap program
            
            let symbolWalker = program.getTypeChecker().getSymbolWalker(SymbolKey.fromSymbol >> symbolMap.ContainsKey >> not)
            let sourceFiles = program.getSourceFiles()
            let external,nonExternal = sourceFiles.AsArray |> Array.partition ts.isExternalModule
            let handleSymbol (symbol: Ts.Symbol) =
                let nodes =
                    symbol.getDeclarations()
                    |> Option.map _.AsArray
                    |> Option.defaultValue [||]
                    |> fun decls ->
                        if symbol.valueDeclaration.IsSome then
                            decls |> Array.append [|symbol.valueDeclaration.Value|]
                        else decls
                    |> Array.distinctBy NodeKey.fromNode
                let symbolTracer = getOrAddSymbol SymbolTracer.create program symbol
                let nodeTracers = nodes |> Seq.map (getOrAddNode NodeTracer.create program)
                match symbolNodeMap.TryGetValue(symbolTracer.SymbolKey) with
                | true, nodeKeyHashSet ->
                    nodeTracers |> Seq.iter (_.NodeKey >> nodeKeyHashSet.Add >> ignore)
                | _ ->
                    let hashSet = nodeTracers |> Seq.map _.NodeKey |> Seq.distinct |> HashSet
                    symbolNodeMap[symbolTracer.SymbolKey] <- hashSet
            let handleType (typ: Ts.Type) =
                let typeTracer = getOrAddType TypeTracer.create program typ
                typ.getCanonicalSymbol()
                |> Option.orElse (typ?symbol |> Option.map (ISymbol.create program >> ISymbol.toSymbol))
                |> Option.iter handleSymbol
            let handleWalkResult (walkResult: WalkerResult) =
                walkResult.visitedSymbols
                |> Seq.iter handleSymbol
                walkResult.visitedTypes
                |> Seq.iter handleType
            nonExternal
            |> Seq.map (Node.SourceKind.create program)
            |> Seq.choose (function SourceKind.Script v -> Some v | _ -> None)
            |> Seq.collect (Node.Script.getSymbolLocals >> Seq.map _.Value.Value)
            |> Seq.iter handleSymbol
            external
            |> Array.iter (
                checker.getSymbolAtLocation
                >> Option.defaultWith (fun () -> failwith "Could not get symbol at location for external source file")
                >> fun sym ->
                    sym.exports
                    |> Option.iter (_.values() >> Seq.toArray >> Array.iter (symbolWalker.walkSymbol >> handleWalkResult))
                    symbolWalker.walkSymbol sym
                    |> handleWalkResult
                    handleSymbol sym
                )
    ...
    module TypeTracer =
        ...
        let UniqueESSymbol = NarrowedTracer.createFunctionSet<TypeUniqueESSymbolTracer, TypeTracer, _, _> <| function
            ...
            | _ -> None
        let Interface = NarrowedTracer.createFunctionSet<TypeInterfaceTracer, TypeTracer, _, _> <| function
            | Kind.Structural (Structural.Interface typ) -> Choice1Of4 typ |> Some
            | Kind.Structural (Structural.Class typ) -> Choice2Of4 typ |> Some
            | Kind.Structural (Structural.TypeReference (TypeReference.Interface typ)) -> Choice4Of4 typ |> Some
            | Kind.Structural (Structural.TypeReference (TypeReference.Class typ)) -> Choice3Of4 typ |> Some
            | _ -> None
    module SymbolTracer =
        open Type
        ...
        let Parameter = NarrowedTracer.createFunctionSet<SymbolParameterTracer, SymbolTracer, _, _> <| function
            ...
            //     | _ -> None
            | _ -> None
        let inline private typeNodeIsUniqueESSymbol symbol =
                    (Node.Type.create (ISymbol.program symbol) >> function
                        | Type.TypeOperator(TypeOperator.Unique(Type.Keyword(TypeKeyword.Symbol, _)), _) -> true
                        | _ -> false
                        )
        let UniqueESSymbol = NarrowedTracer.createFunctionSet<SymbolUniqueESSymbolTracer, SymbolTracer, _, _> <| function
            | Kind.Variable symbol ->
                let canonicalIsUniqueESSymbol =
                    Symbol.canonicalDeclaration symbol
                    |> _.``type``
                    |> Option.exists (typeNodeIsUniqueESSymbol symbol)
                if canonicalIsUniqueESSymbol then
                    Choice1Of2 symbol |> Some
                else None
            | Kind.Property symbol ->
                let canonicalIsUniqueESSymbol =
                    Symbol.canonicalDeclaration symbol
                    |> function
                        | PropertyKind.Class node ->
                            match node.Value.name with
                            | Patterns.Node.PropertyNamePatterns.ComputedPropertyName _ ->
                                node.Value.``type``
                                |> Option.exists (typeNodeIsUniqueESSymbol symbol)
                            | _ -> false
                        | PropertyKind.Type node ->
                            node.Value
                            |> (ISymbol.checker symbol).getTypeAtLocation
                            |> Type.Kind.create (ISymbol.program symbol)
                            |> function
                                | Type.Kind.Primitive (Primitive.Literal (Literal.UniqueESSymbol _)) -> true
                                | _ -> false
                if not canonicalIsUniqueESSymbol then None else
                Choice2Of2 symbol |> Some
            | _ -> None
        let Interface = NarrowedTracer.createFunctionSet<SymbolInterfaceTracer, SymbolTracer, _, _> <| (
            Symbol.foldToTransientKind >> function
                | Transient.Kind.Interface symbol -> Some symbol
                | Transient.Kind.Function symbol -> Some symbol
                | Transient.Kind.Parameter symbol -> Some symbol
                | Transient.Kind.Variable symbol -> Some symbol
                | Transient.Kind.Property symbol -> Some symbol
                | Transient.Kind.Class symbol -> Some symbol
                | Transient.Kind.ValueModule symbol -> Some symbol
                | Transient.Kind.NamespaceModule symbol -> Some symbol
                | Transient.Kind.Method symbol -> Some symbol
                | Transient.Kind.GetAccessor symbol -> Some symbol
                | Transient.Kind.SetAccessor symbol -> Some symbol
                ...
                | Transient.Kind.TypeAlias _
                | Transient.Kind.Unknown _ -> None
            )
    ...
    module NodeTracer =
        ...
        let UniqueESSymbol = NarrowedTracer.createFunctionSet<NodeUniqueESSymbolTracer, NodeTracer, _, _> <| function
            | Node.Kind.DeclarationOrType declKind ->
                ...
                | _ -> None
            | _ -> None
        let Interface = NarrowedTracer.createFunctionSet<NodeInterfaceTracer, NodeTracer, _, _> <| function
                        | Node.Kind.DeclarationOrType (Node.DeclarationKind.Interface node) -> Some node.Value
                        | _ -> None
    ...
    module SymbolUniqueESSymbolTracer =
        let private nodeSymbol = SymbolTypeKey.create<NodeUniqueESSymbolTracer> "NodeUniqueESSymbolTracer"
        let private typeSymbol = SymbolTypeKey.create<TypeUniqueESSymbolTracer> "TypeUniqueESSymbolTracer"
        let inline tryFromSymbolTracer tracer =
        ...
        let node (tracer: SymbolUniqueESSymbolTracer) =
            let fetch = fun () ->
                ...
                | Choice2Of2 symbol ->
                    ...
                    |> NodeTracer.create tracer.program
                    |> NodeUniqueESSymbolTracer.unsafeFromNodeTracer
            SymbolTypeKey.accessOrInit nodeSymbol fetch tracer
        let type_ (tracer: SymbolUniqueESSymbolTracer) =
            let fetch = fun () ->
                match SymbolTracer.UniqueESSymbol.value tracer with
                | Choice1Of2 symbol ->
                    Symbol.canonicalDeclaration symbol
                    |> _.``type``
                    |> Option.get
                    |> (ISymbol.checker symbol).getTypeFromTypeNode
                    |> TypeTracer.create (ISymbol.program symbol)
                    |> TypeUniqueESSymbolTracer.unsafeFromTypeTracer
                | Choice2Of2 symbol ->
                    match Symbol.canonicalDeclaration symbol with
                    | PropertyKind.Class parentInlinedProgram ->
                        parentInlinedProgram.Value.``type``
                        |> Option.defaultWith (fun () -> failwith "Cannot get the type of a unique symbol if it is not a type node")
                        |> (ISymbol.checker symbol).getTypeFromTypeNode
                        |> TypeTracer.create (ISymbol.program symbol)
                        |> TypeUniqueESSymbolTracer.unsafeFromTypeTracer
                    | PropertyKind.Type parentInlinedProgram ->
                        ISymbol.checker symbol
                        |> _.getTypeAtLocation(parentInlinedProgram.Value)
                        |> TypeTracer.create (ISymbol.program symbol)
                        |> TypeUniqueESSymbolTracer.unsafeFromTypeTracer
            SymbolTypeKey.accessOrInit typeSymbol fetch tracer

    // This should only be created from the UniqueESSymbolType tracer
    module NodeUniqueESSymbolTracer =
        let private typeSymbol = SymbolTypeKey.create<TypeUniqueESSymbolTracer> "TypeUniqueESSymbolTracer"
        let private symbolSymbol = SymbolTypeKey.create<SymbolUniqueESSymbolTracer> "SymbolUniqueESSymbolTracer"
        ...
        let inline unsafeFromNodeTracer tracer =
            ...
            |> Option.defaultWith (fun () -> failwith "Cannot narrow a node tracer if it doesn't exist")
        let node (tracer: NodeUniqueESSymbolTracer) =
            NodeTracer.UniqueESSymbol.value tracer
        let type_ tracer =
            let fetch = fun () ->
                match node tracer with
                | Choice1Of2 node ->
                    match node with
                    | PropertyKind.Class node ->
                        node.Value.``type``
        ...
        let symbol tracer =
            let fetch = fun () ->
                type_ tracer
                |> TypeUniqueESSymbolTracer.symbol
            SymbolTypeKey.accessOrInit symbolSymbol fetch tracer
    module TypeUniqueESSymbolTracer =
        ...
        let declaration tracer =
            symbol tracer
            |> SymbolUniqueESSymbolTracer.node
    
    module TypeInterfaceTracer =
        let inline tryFromTypeTracer tracer =
            TypeTracer.Interface.tryGet tracer
            |> Option.map (fun t -> t :?> TypeInterfaceTracer)
    module NodeInterfaceTracer =
        let inline tryFromNodeTracer tracer =
            NodeTracer.Interface.tryGet tracer
            |> Option.map (fun t -> t :?> NodeInterfaceTracer)
        let inline unsafeFromNodeTracer tracer =
            tryFromNodeTracer tracer
            |> Option.defaultWith (fun () -> failwith "Cannot narrow a node tracer if it doesn't exist")
        let tryCreate program (node: Ts.Node) =
            NodeTracer.create program node
            |> tryFromNodeTracer
        let inline narrowedValue (tracer: NodeInterfaceTracer) = tracer |> NarrowedTracer.value
        let inline node tracer = narrowedValue tracer
        let type_ tracer =
            node tracer
            |> tracer.checker.getTypeAtLocation
            |> TypeTracer.create tracer.program
    module SymbolInterfaceTracer =
        let inline tryFromSymbolTracer tracer =
            SymbolTracer.Interface.tryGet tracer
            |> Option.map (fun t -> t :?> SymbolInterfaceTracer)
        let inline unsafeFromSymbolTracer tracer =
            tryFromSymbolTracer tracer
            |> Option.defaultWith (fun () -> failwith "Cannot narrow a symbol tracer if it doesn't exist")
        let tryCreate program (symbol: Ts.Symbol) =
            SymbolTracer.create program symbol
            |> tryFromSymbolTracer
        let inline narrowedValue (tracer: SymbolInterfaceTracer) = SymbolTracer.Interface.value tracer
        let inline symbol tracer =
            narrowedValue tracer
        let node tracer =
            narrowedValue tracer
    ...
    module Wrapped =
        module Identifier =
            module Builder =
                ...
                let initWithSymbol symbol: Builder = withSymbol symbol empty
                let initWithNode node: Builder = withNode node empty
                let initWithType typeKey: Builder = withType typeKey empty
                let build builder: UniqueIdentifiers.CompositeKey =
...
module CoreExtensions =
    ...
    type Transient.GetAccessor with
        ...
        member inline this.canonical = Symbol.canonicalDeclaration this
        member inline this.canonicalWithType = Symbol.canonicalDeclarationAndType this
    type GetAccessorDeclaration with
        member this.valueDeclaration = Symbol.valueDeclaration this
    type Transient.SetAccessor with
        ...
        member inline this.canonical = Symbol.canonicalDeclaration this
        member inline this.canonicalWithType = Symbol.canonicalDeclarationAndType this
    type SetAccessorDeclaration with
        member this.valueDeclaration = Symbol.valueDeclaration this
