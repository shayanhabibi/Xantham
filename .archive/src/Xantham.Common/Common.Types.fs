namespace Xantham.Annotations

[<MeasureAnnotatedAbbreviation>]
type string<[<Measure>] 'U> = string

module Measures =
    let inline annotate<[<Measure>] 'U> (input: string) = unbox<string<'U>> input
    let inline strip<[<Measure>] 'U> (input: string<'U>) = unbox<string> input
    let inline replace<[<Measure>] 'O, [<Measure>] 'U> (input: string<'O>) = strip input |> annotate<'U>
    let inline map fn (input: string<'U>): string<'U> = strip input |> fn |> annotate
    let inline mapf fn (input: string<'U>): 'T = strip input |> fn
...
/// <summary>
/// The interfacing type is an erased wrapper around the provided underlying type.
/// This can be used to automatically bind members such as <c>_.Value</c> which
/// will unwrap the interfacing type to its underlying type.
/// </summary>
[<Interface>]
type IUnwrappable<'ErasedUnderlyingType> = interface end
[<Interface>]
type IIdentityKey<'T> =
    inherit IUnwrappable<int>
...
module NodeKey =
    ...
    let encode: Encoder<NodeKey> = unbox >> Encode.int
    let decode: Decoder<NodeKey> = Decode.int >> unbox
module SymbolKey =
    let inline createWith (i: int) = SymbolKey i
    let encode: Encoder<SymbolKey> = unbox >> Encode.int
    let decode: Decoder<SymbolKey> = Decode.int >> unbox
#else
open Thoth.Json.Net
[<AutoOpen>]
...
module TypeKey =
    let inline createWith (i: int) = TypeKey i
    let encode: Encoder<TypeKey> = _.Value >> Encode.int
    let decode: Decoder<TypeKey> = Decode.int |> Decode.map TypeKey
module NodeKey =
    let inline createWith (i: int) = NodeKey i
    let encode: Encoder<NodeKey> = _.Value >> Encode.int
    let decode: Decoder<NodeKey> = Decode.int |> Decode.map NodeKey
module SymbolKey =
    let inline createWith (i: int) = SymbolKey i
    let encode: Encoder<SymbolKey> = _.Value >> Encode.int
    let decode: Decoder<SymbolKey> = Decode.int |> Decode.map SymbolKey
#endif
