...
let private resolveIn (entries: RecipeEntry list) =
    resolveEntryFiles repoRoot { Entries = entries }
...
let integrationTests =
    testList "Recipe.resolveEntryFiles (integration, repo node_modules)" [
        testCase "module '.' resolves through the exports map to the types file" <| fun _ ->
            match resolveIn [ { Package = "agents"; Kind = Module; Root = None; Entries = [ "." ]; Crawl = true } ] with
            | Ok [ r ] ->
        ...
        testCase "module subpath './mcp' resolves distinctly from '.'" <| fun _ ->
            match resolveIn [ { Package = "agents"; Kind = Module; Root = None; Entries = [ "."; "./mcp" ]; Crawl = true } ] with
            | Ok [ root; mcp ] ->
        ...
        testCase "ambient-root resolves to an ABSOLUTE existing file" <| fun _ ->
            match resolveIn [ { Package = "@cloudflare/workers-types"; Kind = AmbientRoot; Root = Some "latest/index.ts"; Entries = []; Crawl = true } ] with
            | Ok [ r ] ->
        ...
        testCase "crawl=false entries resolve to NOTHING (policy-holders)" <| fun _ ->
            match resolveIn [ { Package = "zod"; Kind = Module; Root = None; Entries = [ "." ]; Crawl = false } ] with
            | Ok [] -> ()
            | other -> failwith $"unexpected: %A{other}"
        ...
        testCase "PINNED: the MCP SDK root export is a lie (declared but unshipped)" <| fun _ ->
            ...
            match resolveIn [ { Package = "@modelcontextprotocol/sdk"; Kind = Module; Root = None; Entries = [ "." ]; Crawl = true } ] with
        ...
        testCase "MCP SDK '.js' touchpoint subpaths resolve to their .d.ts twins" <| fun _ ->
            match resolveIn [ { Package = "@modelcontextprotocol/sdk"; Kind = Module; Root = None; Entries = [ "./types.js"; "./server/mcp.js" ]; Crawl = true } ] with
            | Ok [ types; mcp ] ->
        ...
        testCase "errors accumulate across ALL failing resolutions" <| fun _ ->
            let missing sub = { Package = "agents"; Kind = Module; Root = None; Entries = [ sub ]; Crawl = true }
