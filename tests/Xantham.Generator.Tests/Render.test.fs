...
let renderTests =
    testList "render" [
        ...
        testCase "the manifest reports per-symbol tiers with pass provenance" <| fun _ ->
            ...
            // One symbol per line, and every field the aggregate does not carry: the symbol, its
            // declaration file, its worst tier, and each finding under both its identities.
            let expectedSymbols =
                String.concat
                    "\n"
                    [ "{\"name\":\"Options\",\"file\":\"index.d.ts\",\"tier\":\"widened\",\"findings\":["
                      + "{\"name\":\"TR.TypeNotResolved\",\"key\":\"TR002\",\"pass\":\"shape-interfaces\","
                      + "\"tier\":\"widened\",\"symbol\":\"Options.legacy\",\"fields\":{\"reason\":\"legacy\"},"
                      + "\"message\":\"type not resolved (legacy); widened to obj\"}]}"
                      "{\"name\":\"Dropped\",\"tier\":\"escape\",\"findings\":["
                      + "{\"name\":\"AC.ExportNotRepresented\",\"key\":\"AC001\",\"pass\":\"audit-coverage\","
                      + "\"tier\":\"escape\",\"symbol\":\"Dropped\","
