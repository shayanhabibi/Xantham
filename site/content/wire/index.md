...
Supports `net10.0`, `net8.0`, `netstandard2.1`. Only dependency is `System.Text.Json`{fsharp}.
...
- `Tsc.locate`{fsharp} returns `string option`{fsharp}. 

- `XANTHAM_TSGO_EXE`, if set to an existing file, wins over the search. 

- Treat `None`{fsharp} as "the caller has not run `npm install`{bash}" and say so — it is the single
