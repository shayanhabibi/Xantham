...
module PackageVersion =
    ...
    /// The `typescript` devDependency pinned in the root `package.json`.
    let private packagePin () =
        using (JsonDocument.Parse(Repo.FileSystem.``package.json``.OpenRead()))
        <| fun doc ->
            match doc.RootElement.TryGetProperty "devDependencies" with
            | true, devDeps ->
                match devDeps.TryGetProperty "typescript" with
                | true, tsElement -> Ok(tsElement.GetString())
                | _ -> Error "no typescript dev dependency"
            | _ -> Error "no devDependencies"

    let private specFile = Repo.FileSystem.src.``Xantham.Cli``.``Spec.fs``.FullName
...
"""
                    |> File.writeString false specFile))
        }

    /// Fails when `Spec.tscVersion` in `src/Xantham.Cli/Spec.fs` differs from the `package.json`
    /// pin. The CLI ships the literal, so a packed tool with a stale one downloads the wrong
    /// compiler.
    let checkCliPackageVersion =
        stage "check-cli-package-version" {
            run (fun _ ->
                packagePin ()
                |> Result.bind (fun version ->
                    let literal = $"let tscVersion = \"{version}\""

                    if (File.readAsString specFile).Contains literal then
                        Ok()
                    else
                        Error
                            $"src/Xantham.Cli/Spec.fs does not pin typescript {version} (package.json). Run `dotnet fsi build.fsx -- generate cli-tsc-version` and commit the result."))
        }
...
module Stages =
    ...
    /// Formats the tree in place; on CI, fails on unformatted files instead of rewriting them.
    let format =
        input {
            let! quick = Options.quick
        ...
        input {
            ...
            return
                stage "pack" {
                    quiet
                    workingDir Repo.FileSystem.``.``
                    whenSome cliProject (fun _ -> PackageVersion.checkCliPackageVersion)
                    // `publish` pushes every package in `bin/`, so a package left by an earlier
                    // pack must not survive into this one.
                    run (fun _ -> Shell.cleanDir (System.IO.Path.Combine(__SOURCE_DIRECTORY__, "bin")))
