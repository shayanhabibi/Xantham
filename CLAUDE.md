...
- **`Utils/String.fs`** — String utility helpers
- **`Utils/Array.fs`** — Array utility helpers
- **`Utils/Utils.fs`** — `[<AutoOpen>]` module. `List<'T>.AsArray` JS-unbox extension. `Log` module wrapping `JS.console.error` with chalk colour helpers (`success`/`info`/`warn`/`error`/`debug`). `Report` module with `Report.readerError` overloads for `Ts.Type` (symbol-name based) and `Ts.Node` (source-file `file(line,col)` IDE-friendly format) using `CallerFilePath`/`CallerLineNumber` for automatic origin attribution.
- **`Utils/TypeScript.Extensions.fs`** — Extension methods on TypeScript API types
- **`Types/TypeKeyExtensions.fs`** — `TypeKey` and `Ts.Type` extension members; defines primitive `TypeKey` sentinels (`TypeKey.String`, `.Int`, `.Float`, `.Bool`, `.Null`, `.Unit`, `.Any`, etc.)
...
- **`Types/ReactiveBuilders.fs`** — Signal-based mirror of the builder types. `Signal<TypeKey>` (aliased `TypeSignal`) replaces `ref<TypeKey>`; `Signal<SXxx voption>` replaces `ref<TsXxxBuilder voption>`; docs collapse to `TsComment list`. Each `SXxx` type's `.Build()` returns the final `TsXxx` type directly (no Builder-suffixed intermediate). The two primary reactive builder DUs are `SType` (mirrors `TsType`; cases: GlobalThis, Tuple, Interface, Primitive, Predicate, Literal, TypeLiteral, TypeParameter, IndexAccessType, Index, TypeReference, Array, Enum, EnumCase, SubstitutionType, Conditional, Class, Union, Intersection, Optional, TemplateLiteral) and `STsExportDeclaration` (mirrors `TsExportDeclaration`; cases: Interface, Class, Enum, Variable, Function, TypeAlias, Module). `TypeStore.Builder` is `PendingSignal<SType>`; `ExportStore.Builder` is `PendingSignal<STsExportDeclaration>`. Five builder types implement `IOverloadable`: `SMethodBuilder`, `SCallSignatureBuilder`, `SConstructSignatureBuilder`, `SConstructorBuilder`, and `SFunctionBuilder` — the latter wrapped via `TsOverloadableConstruct.Create` in `STsExportDeclaration.Function.Build()`.
- **`Types/Reader.fs`** — Defines `TypeScriptReader` record and `TypeScriptReader.create`/`TypeScriptReader.createFor` factory functions that create the TypeScript compiler program with bundler module resolution. Reader composition utilities and the `typeScriptReader` global reference.
...
- **`Types/Reader.Extensions.fs`** — Extension methods on `TypeScriptReader`
- **`Types/XanTag.Extensions.fs`** — Extension methods on `XanthamTag`
- **`Reading/Prelude.fs`** — Shared reader state and higher-level reading helpers: global stack, `typeScriptReader` reference, `RefKey`/`RefMember` helpers for deferred node registration, `TypeStore.Create` helpers, `addToMemory`/`tryGetOrRegisterStore` for the signal cache, `libCacheMemoryHandler` (adds lib-file types to `libCache` to suppress noise in missing-builder logs), `tagPrimitives`.
- **`Reading/JSDocTags.fs`** — Reads JSDoc tags from nodes
- **`Reading/TupleTypeMember.fs`** — Reads tuple type members
...
- **`Reading/Member.fs`** — `resolveToMemberBuilder`: given a `Ts.TypeElement`, creates/retrieves its `XanthamTag`, pushes to stack if unvisited, and returns a `Signal<SMemberBuilder voption>` by mapping the tag's `AstNodeBuilder` signal. Used by `mergeMembersIntoWinner` and individual node readers when a node's members need to be resolved reactively.
- **`Reading/Dispatch/TypeDeclaration.fs`** — Dispatch handler for `XanTagKind.TypeDeclaration` cases (Interface, TypeAlias, FunctionDeclaration, VariableStatement, Enum, TypeParameter, HeritageClause, Class, Module/Namespace, etc.)
- **`Reading/Dispatch/MemberDeclaration.fs`** — Dispatch handler for `XanTagKind.MemberDeclaration` cases (PropertySignature, MethodSignature, Parameter, Constructor, CallSignature, ConstructSignature, IndexSignature, GetAccessor, SetAccessor, Property, Method, etc.)
- **`Reading/Dispatch/LiteralTokenNode.fs`** — Dispatch handler for literal token nodes
- **`Reading/Dispatch/TypeFlagLiteral.fs`** — Dispatch handler for `TypeFlags` literal cases
- **`Reading/Dispatch/TypeFlagObject.fs`** — Dispatch handler for `TypeFlags.Object` cases (Anonymous, Interface, Class, Mapped, Tuple, Instantiated, Reference)
- **`Reading/Dispatch/TypeFlagPrimary.fs`** — Dispatch handler for primary `TypeFlags` cases
- **`Reading/Dispatch/TypeNode.fs`** — Dispatch handler for `XanTagKind.TypeNode` cases
- **`Reading/Dispatch/ModulesAndExports.fs`** — Intentionally ignores all import/export nodes (zero information loss — the checker resolves these transparently)
- **`Reading/Dispatcher.fs`** — Dispatches a `XanthamTag` to the appropriate `Reading/Dispatch/*.fs` handler based on `XanTagKind`. Pattern: `Ignore` → skip; `tryGetOrRegisterStore` returns `None` (already cached) → skip; otherwise dispatch.
...
- **`Read.fs`** — Stack-based traversal engine (replaces tail recursion since JS has no TCO). Stack entries: `TypeNode`, `Declaration`, `TypeKind`, `JsDocTag`, `DelayedFunc`. Typed result records: `IRResult { Identity; Key; Node }` and `IRResultDuplicates<'phantom> { Key; Results }` (phantom type `idPrioritySorted` witnesses sort completion). After traversal, `readAndWrite` runs a four-phase duplicate-resolution pipeline: (1) `exciseDuplicateKeys` — split `NonDuplicates` from `DuplicateGroups`; (2) `sortResultGroups` — sort each group's `Results` by identity priority (`DeclarationPosition > Symbol > AliasSymbol > Id`), returning `IRResultDuplicates<idPrioritySorted>`; (3) `mergeOverloads` — wraps overloadable entries into `TsOverloadableConstruct<'T>`; (4) `mergeMembersIntoWinner` — for Interface/Class groups, appends all unique members from non-winner entries into the winner node; (5) `filterConflictDuplicatesOnly` — logs `[COLLIDE]` warnings only for genuine node conflicts. Diagnostic log markers: `[CIRCREF]` circular reference, `[MISSREF]` missing builder, `[COLLIDE]` key conflict. `Internal` module exposes the pipeline phases (`initialise`, `getAndPrepareExports`, `assembleResults`, `exciseDuplicateKeys`, `sortResultGroups`, `mergeOverloads`, `mergeMembersIntoWinner`, `filterConflictDuplicatesOnly`, `resolveDuplicates`, `prune`) for use by the test harness.
...
`Ts.Signature.getDeclaration()` can return `CallSignatureDeclaration`, `FunctionTypeNode`, `MethodSignature`, `MethodDeclaration`, `ConstructorTypeNode`, or `ConstructorDeclaration` — not only the explicit signature declaration kinds. `Reading/Dispatch/TypeFlagObject.fs` handles all of these for anonymous/interface object types. `FunctionTypeNode` and `ConstructorTypeNode` are built inline (they are not `Declaration` nodes and cannot go through `RefMember.fromDeclaration`).

### Xantham.Decoder

- **`Types/Decoding.Types.fs`** — Types used during JSON decoding
- **`Types/Name.fs`** — Name resolution types and utilities
- **`Types/Frozen.Extensions.fs`** — Extension methods for frozen/immutable collections
- **`Types/CodedTypeKey.fs`** — TypeKey encoding/decoding helpers for JSON transport
...
- **Fable.Mocha 2.17.0** — Testing framework for `Xantham.Fable.Tests`
...
- `TypeScript.fs` (the TSC API binding) is auto-generated via ts2fable — do not manually edit it; use `Utils/TypeScript.Extensions.fs` for extensions
...
- `Types/ReactiveBuilders.fs` is compiled after `Types/XanTag.fs`, `Types/Signal.fs`, and `Types/ModuleMap.fs` in the fsproj
- `Types/Reader.fs` is compiled after `Types/ReactiveBuilders.fs` per the current fsproj ordering
- `Types/GuardedData.fs` is compiled after `Types/Reader.fs`, before `Types/Reader.Extensions.fs`
...
- **Duplicate-resolution pipeline order**: `exciseDuplicateKeys` → `sortResultGroups` → `mergeOverloads` → `mergeMembersIntoWinner` → `filterConflictDuplicatesOnly` → `resolveDuplicates`. Each phase operates on `IRResultDuplicates<idPrioritySorted>` after sorting, so callers can rely on `Results[0]` being the highest-priority winner.
- **Overload merging** (`mergeOverloads`): groups duplicate-key entries by overloadability (the five `IOverloadable` types), wraps them into `TsOverloadableConstruct.Create` (producing `NoOverloads` or `Overloaded Set`), and replaces the winner entry. Non-overloadable duplicates pass through. `healthCheckNode` and `findUnknownFields` both iterate overloads via `.Values`. Consumer code uses `.Values` / `.ToList()` / `.ToSeq()` to process all overloads, or `.ValueOrHead` for backward-compatible single-value access.
- **Member merging** (`mergeMembersIntoWinner`): for Interface and Class groups, collects all members from non-winner entries that are not already present in the winner node, and appends them. Fixes the case where two declarations of the same interface (same `TypeKey`) would silently lose the members of lower-priority duplicates. The earlier `&& result.Identity.IsDeclarationPosition` guard on the merge partition was a bug — it caused member merging to be skipped for non-`DeclarationPosition` identities even when the nodes were structurally compatible Interface/Class nodes.
