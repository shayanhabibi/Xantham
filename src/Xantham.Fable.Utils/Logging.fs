...
/// </remarks>
let private printfFmtSpecPattern =
    """%"""
    // flags
    + """(0|-|\+| )*"""
    // width
...
/// </returns>
let private processLogMsgParams (format: Format<'Printer, 'State, 'Residue, 'Result, 'Tuple>) : string option list * Format<'Printer, 'State, 'Residue, 'Result, 'Tuple> * string =
...
let rec private collectArgsDynamic (n: int) (collected: System.Collections.Generic.List<obj>) (f: obj) : obj =
    if n <= 0 then f
    else
        let f' = f :?> obj -> obj
        (fun (x: obj) ->
...
let private klogf (continuation: LogEntry -> 'Result) (format: Format<'T, unit, string, 'Result, 'Tuple>) : 'T =
...
/// </remarks>
let private formatEntryAsJson (level: LogLevel) (category: string voption) (entry: LogEntry) (err: Exception) : string =
    let data =
        entry.ArgNames
        |> List.mapi (fun i name ->
            let key =
                name
                |> Option.either _.TrimStart('@') (fun _ -> sprintf "arg%d" i)
...
let inline private getCategory<'T> = typeof<'T>.Name
/// <summary>
...
type Logger(?console: JS.Console, ?prependLevelToEntries: bool, ?logLevel: LogLevel, ?category: string, ?prependCategoryToEntries: bool) =
    let prependLevelToEntries = defaultArg prependLevelToEntries true
    let category = category |> Option.filter ((<>) getCategory<obj>)
    let prependCategoryToEntries = defaultArg prependCategoryToEntries true && category.IsSome
    ...
    member _.PrependCategoryToEntries = prependCategoryToEntries
    ...
    default this.WriteEntry (level, _entry, message, err) =
        let log m =
            ...
            | _ -> console.log m
        let m =
            let level = Enum.GetName(typeof<LogLevel>, level).ToUpper()
            match prependLevelToEntries, prependCategoryToEntries with
            | true, true -> sprintf "[%s] [%s] " level category.Value
            | true, _ | _, true ->
                sprintf "[%s] " (if prependLevelToEntries then level else category.Value)
            | _ -> ""
            + message
        log m
...
/// <param name="logLevel">The minimum level emitted; defaults to <c>Debug</c> in debug builds, <c>Warning</c> otherwise.</param>
/// <category>Loggers</category>
type ConsoleLogger<'Category>(?prependLevelToEntries: bool, ?logLevel: LogLevel, ?category: string) =
    inherit Logger(JS.console, ?prependLevelToEntries = prependLevelToEntries, ?logLevel = logLevel, ?category = category)
    /// This logger upcast to <see cref="T:ILogger"/>.
    member inline this.ILogger = this :> ILogger
    static member inline Create<'Category>(?prependLevelToEntries: bool, ?logLevel: LogLevel) =
        ConsoleLogger<'Category>(?prependLevelToEntries = prependLevelToEntries, ?logLevel = logLevel, category = typeof<'Category>.Name)

type ConsoleLogger = ConsoleLogger<obj>
...
/// </summary>
/// <category>Loggers</category>
type TextWriterLogger<'Category> private (_console, writeStream, prependLevelToEntries: bool, logLevel: LogLevel, ?category: string) =
    inherit Logger(_console, prependLevelToEntries = prependLevelToEntries, logLevel = logLevel, ?category = category)
    ...
    override this.WriteEntry (level, entry, message, err) =
        match entry with
        | Some e -> this.Console.log (formatEntryAsJson level ValueNone e err)
    ...
    /// <param name="category"/>
    new(file: string, ?prependLevelToEntries: bool, ?logLevel: LogLevel, ?category: string) =
        let prependLevelToEntries = defaultArg prependLevelToEntries true
        ...
        let myConsole =
            ...
            |> unbox<JS.Console>
        TextWriterLogger(myConsole, writeStream, prependLevelToEntries = prependLevelToEntries, logLevel = logLevel, ?category = category)
    static member inline Create<'Category>(file: string, ?prependLevelToEntries: bool, ?logLevel: LogLevel) =
        TextWriterLogger<'Category>(file, ?prependLevelToEntries = prependLevelToEntries, ?logLevel = logLevel, category = getCategory<'Category>)

type TextWriterLogger = TextWriterLogger<obj>
...
type Log(logger: ILogger, ?rootPath: string) =
    ...
    static let defaultLog = Log(ConsoleLogger.Create<Log>())
    static member Default = defaultLog
    static member inline Create<'Category>(logger, ?rootPath) = Log(logger, ?rootPath = rootPath)
    /// The wrapped logger.
