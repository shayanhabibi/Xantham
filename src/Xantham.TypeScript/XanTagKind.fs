...
module Internal =
    ...
    let moduleExportSetMap: Dictionary<Ts.SyntaxKind, obj -> ModulesAndExports> =
        Dictionary [
            ...
            Ts.SyntaxKind.NamespaceExport >=> ModulesAndExports.NamespaceExport
            Ts.SyntaxKind.NamespaceExportDeclaration >=> ModulesAndExports.NamespaceExportDeclaration
        ]
...
type MemberDeclaration with
    ...
    /// Totality over real class &amp; interface members is proven by <b>XTK-4</b> (Program.test.fs); guard
    ...
    /// <remarks>Proof <b>XTK-4</b> (Program.test.fs) asserts this returns <c>true</c> for every class &amp; interface member in the corpus.</remarks>
...
type TypeDeclaration with
    ...
    member inline this.Symbol: Ts.Symbol voption =
        ...
        if node?symbol then
            ...
            Logging.Log.Default.logfe "TypeDeclaration.Symbol: unexpected failure to find symbol for %s{typeDeclaration}. Please raise an issue."  (this.ToString())
            ValueNone
type TopLevelStatements with
    ...
    static member Create(xanTagKind: XanTagKind) =
        match xanTagKind.UnderlyingValue with
        | Choice2Of2 node when TopLevelStatements.IsTopLevelStatementKind node ->
            TopLevelStatements.Create node
            |> ValueSome
        | _ -> ValueNone
    static member inline FromXanTagKind (xanTagKind: XanTagKind) = TopLevelStatements.Create xanTagKind
    member inline this.AsTypeDeclaration =
        if TypeDeclaration.IsTypeDeclarationKind this.Value then
            TypeDeclaration.Create this.Value
            |> ValueSome
        else ValueNone
    member inline this.AsModuleAndExport =
        if not <| ModulesAndExports.IsModulesAndExportsKind this.Value then ValueNone else
        this.Value
...
type TopLevelExportSymbolDeclarations with
    ...
    static member Create(xanTagKind: XanTagKind) =
        match xanTagKind.UnderlyingValue with
        | Choice2Of2 node when TopLevelExportSymbolDeclarations.IsTopLevelExportDeclarationKind node ->
            TopLevelExportSymbolDeclarations.Create node
            |> ValueSome
        | _ -> ValueNone
    static member inline FromXanTagKind (xanTagKind: XanTagKind) = TopLevelExportSymbolDeclarations.Create xanTagKind
    member inline this.AsTypeDeclaration =
        if TypeDeclaration.IsTypeDeclarationKind this.Value then
            TypeDeclaration.Create this.Value
            |> ValueSome
...
type TopLevelLocalSymbolDeclarations with
    ...
    static member Create(xanTagKind: XanTagKind) =
        match xanTagKind.UnderlyingValue with
        | Choice2Of2 node when TopLevelLocalSymbolDeclarations.IsTopLevelLocalDeclarationKind node ->
            TopLevelLocalSymbolDeclarations.Create node
            |> ValueSome
        | _ -> ValueNone
    static member inline FromXanTagKind (xanTagKind: XanTagKind) = TopLevelLocalSymbolDeclarations.Create xanTagKind
    member inline this.AsTypeDeclaration =
        if TypeDeclaration.IsTypeDeclarationKind this.Value then
            TypeDeclaration.Create this.Value
            |> ValueSome
...
type XanTagKind with
    ...
    /// (symbol-table value declarations) all hold across the corpus (Program.test.fs). Calling on a node from an
    ...
    static member Create(node: Ts.Node) =
        ...
        then IgnoredNode.Create node // Intentionally ignored — see CLAUDE.md "Removed XanTagKind cases"
            ...
            Logging.Log.Default.logfw "Unrecognized node kind: %s{nodeKind}" node.kind.Name
            Ignore JS.undefined
    static member Create(node: Ts.Type) =
