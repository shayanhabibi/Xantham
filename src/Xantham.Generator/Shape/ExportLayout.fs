...
let ownerOf (runtimePackage: string<importSpecifier>) = function
...
let private normalizeOwner runtimePackage = function
    | AmbientModule specifier when specifier = runtimePackage -> EntryModule
    | owner -> owner

let private rawPreferredPath (runtimePackage: string<importSpecifier>) hasEntryOwner = function
    | EntryModule -> []
...
let preferredPath (runtimePackage: string<importSpecifier>) hasEntryOwner = function
    | EntryModule -> []
...
let allocate (runtimePackage: string<importSpecifier>) declaredNames owners =
    let owners = owners |> List.map (normalizeOwner runtimePackage) |> List.distinct |> List.sortBy ownerKey
    ...
    requested
    |> List.fold (fun (allocated, result) (owner, path, rawPath) ->
        ...
        let conflictIndex =
            ...
            |> List.sort
            |> List.tryHead
        if conflictIndex.IsNone then Set.add path allocated, Map.add owner path result
        else
            let index = Option.get conflictIndex
            let rec claim length =
                let candidate =
                    path
                    |> List.mapi (fun i segment -> if i = index then $"{segment}_{digest owner length}" else segment)
                if Set.contains candidate allocated
                   || Set.contains candidate requestedPaths
                   || conflictsWithDeclaration declared candidate |> Option.isSome then
                    claim (length + 1)
                else candidate
...
let containerName declaredNames owner path =
    ...
    if not (List.contains dotted declaredNames) then dotted
    else
        let rec claim length =
            let candidate = path @ [ $"Exports_{digest owner length}" ] |> String.concat "."
            if List.contains candidate declaredNames then claim (length + 1) else candidate
        claim 12
