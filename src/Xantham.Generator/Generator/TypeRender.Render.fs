...
module TypedNameRender =
    module TypeRefRender =
        let setNullable isNullable (typeRef: TypeRefRender) =
            { typeRef with Nullable = isNullable }
        let nonNullable = setNullable false
        let nullable = setNullable true
        let orNullable isNullable typeRef = if typeRef.Nullable then typeRef else setNullable isNullable typeRef
    /// <summary>
