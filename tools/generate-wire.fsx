...
module TscAst =
    ...
    let private get (url: string) (accept: string option) =
        async {
    ...
    let private resolveRef (repo: string) (ref: string) =
        async {
            if ref.Length = 40 && ref |> Seq.forall Uri.IsHexDigit then
                return ref
            else
                let! body =
    ...
    let sync (ref: string) (check: bool) =
        async {
                ...
                for file in files do
                    ...
                    let current =
                        if File.Exists dest then
                            Some(sha256 (File.ReadAllBytes dest))
                        else
                            None

                    let status =
                        if current = Some digest then
                            "  ok"
                        else
                            changed.Add path
...
module Options =
    ...
    let upstreamRef =
        ...
        |> Input.desc
            "Git ref of microsoft/TypeScript to vendor the AST generator sources from. Defaults to the pin in tools/tsc-ast/upstream.json"
        |> Input.def ""
    ...
    input {
        ...
        return
            stage "generate proto" {
                ...
                run (
                    cmd
                        $"node tools/proto-gen/generate.mjs {typescriptPkgDir} {parserDir} {outputDir}/Proto.generated.fs"
                )
            }
    }
    ...
    input {
        ...
        return
            stage "generate session" {
                ...
                run (
                    cmd
                        $"node tools/session-gen/generate.mjs {typescriptPkgDir} {parserDir} {outputDir}/Session.generated.fs"
                )
            }
    }
    ...
    input {
        ...
        return
            stage "generate browser" {
                ...
                run (
                    cmd
                        $"dotnet fsi tools/browser-gen/generate.fsx {libDir} {outputDir}/BrowserBindingTable.generated.fs {gateDir}/BrowserBindings.fs"
                )
            }
    }
