...
// ────────────────────────────────────────────────────────────────────────────
// Typed.table.generated.fs - the test project's view of the same three facts
// ────────────────────────────────────────────────────────────────────────────
//
// The typed layer is 7,000 generated lines, so its tests are generated too: a tag added
// upstream is covered without anyone remembering to write a case for it. What is emitted here
// is data, not assertions - the assertions live hand-written in `Typed.test.fs`, and read this
// table. Two rows types, because the two are checked against each other:
//
//   - `views`, one row per active pattern in `Patterns`, carrying the kinds the schema gives
//     that tag and the pattern itself, uniformed to `Node<AnyNode> -> int voption` so that 265
//     differently-typed views fit in one array.
//   - `tags`, one row per tag, carrying its kind set, what it was generated from and its
//     declaration order - enough for a test to re-derive which tags should inherit which and
//     check that against the interfaces the assembly actually has.

const tableOut = new Lines();

tableOut.w(
  "module Xantham.TypeScript.Wire.Tests.TypedTable",
  "",
  "// <auto-generated>",
  `//   Generated by tools/tsc-ast/generate-ast.mts from ${lock.repo}@${lock.ref.slice(0, 12)}`,
  "//   at tools/scripts/tsc/ast.json.",
  "//   Do not edit by hand - re-run the generator when the pinned schema moves.",
  "// </auto-generated>",
);
tableOut.blank();
tableOut.w("open Xantham.TypeScript.Wire", "open Xantham.TypeScript.Wire.Patterns");
tableOut.blank();

const kindArray = (kinds: Iterable<string>) =>
  `[| ${[...kinds].map(kind => `SyntaxKind.${fsIdent(kind)}`).join("; ")} |]`;

tableOut.doc("Every view in `Patterns`: its tag's name, the kinds `ast.json` gives that tag, and the");
tableOut.doc("view applied and reduced to the index it narrowed to. A view that declines is");
tableOut.doc("`ValueNone`, so a row states both halves of the claim - which kinds it takes, and that");
tableOut.doc("what comes back is the same node.");
tableOut.w("let views: (string * SyntaxKind[] * (Node<AnyNode> -> int voption))[] =");
tableOut.indent(w => {
  const rows: string[] = [];
  const row = (name: string, kinds: Iterable<string>) => {
    const view = fsIdent(name);
    rows.push(`("${name}", ${kindArray(kinds)},`
      + ` fun node -> match node with | ${view} narrowed -> ValueSome(Node.index narrowed) | _ -> ValueNone)`);
  };

  for (const node of api.nodes()) row(node.name, node.allKinds().map(kind => kind.name));
  for (const guard of nodeAliasGuards) row(guard.name, aliasKinds.get(guard.name)!);
...
tableOut.doc("Every tag: its name, what it was generated from (`node`, `alias`, `token` or `any`),");
tableOut.doc("its position in the generator's declaration order, and its kind set. The last two are");
tableOut.doc("what let a test re-derive the inheritance the emitter should have produced - a tag");
tableOut.doc("widens to an alias whose kinds are a superset, with ties among equal sets broken by");
tableOut.doc("declaration order - and compare that against the interfaces in the built assembly.");
tableOut.w("let tags: (string * string * int * SyntaxKind[])[] =");
tableOut.indent(w => {
  w.w("[|");
  w.indent(g => {
    for (const [name, tag] of tags) {
      g.w(`("${name}", "${tag.sort}", ${tag.order}, ${kindArray(tag.kinds)})`);
    }
  });
  w.w("|]");
});
...
const write = (name: string, lines: Lines, absolute = false) => {
  const file = absolute ? name : path.join(outDir, name);
...
console.log(`out     ${write(path.join(here, "..", "..", "tests", "Xantham.TypeScript.Wire.Tests", "Typed.table.generated.fs"), tableOut, true)}`);
